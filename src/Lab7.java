...
public class Lab7 extends JPanel implements GLEventListener {
	...
	private Texture textureFromResource(String resourceName) throws IOException {
		...
		URL resource_URL = this.getClass().getClassLoader().getResource(resourceName);
		BufferedImage img = ImageIO.read(Objects.requireNonNull(resource_URL));
		Texture texture;
		...
		if (needsRelease) context.release();
